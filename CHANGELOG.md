...
- Integrated `CA\Log` (via `CaLog` facade) for structured logging across all service classes.
- CRL generation events are logged via `CaLog::crlUpdated()` in `CrlGenerator` and `CrlManager`.
- CRL distribution and publish operations are logged via `CaLog::crlUpdated()` in `CrlDistributor`.
- All exceptions in CRL generation and distribution are logged via `CaLog::critical()` before re-throwing.
