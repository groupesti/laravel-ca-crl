...
class CrlGenerator
{
    ...
    public function buildCrl(
        ...
        try {
            ...
            CaLog::crlUpdated($ca->getId(), count($revokedCerts), [
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'crl_generation', 'exception' => $e::class]);

            throw $e;
        } catch (\Throwable $e) {
            $crlException = new CrlException(
                "Failed to build CRL for CA [{$ca->getId()}]: {$e->getMessage()}",
                previous: $e,
            );
            CaLog::critical($crlException->getMessage(), ['operation' => 'crl_generation', 'exception' => $e::class]);
